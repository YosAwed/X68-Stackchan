...
`firmware/include/servo_controller.h` が State 遷移と発話 RMS に合わせて姿勢を更新する。`SERVO_STATE_MOTION_ENABLED` / `SERVO_IDLE_MOTION_ENABLED` / `SERVO_LIPSYNC_MOTION_ENABLED` で動きの粒度を切り替えられる。
...
| Battery ≤ 15% | サーボ更新を抑止。会話パイプラインは継続 |
| Battery > 20% に回復 | 警告解除、サーボ再有効化 (ヒステリシス) |
| State::Idle が 5 分継続 | `face_09` (Zzz) を出してから `M5.Power.deepSleep()`。電源ボタンで復帰 (= リセット → setup 再実行) |

### 重要な注意: サーボ電源

サーボを CoreS3 内蔵 LiPo だけで駆動すると、突入電流で電圧が落ちて本体リセットの危険がある。Takao Base 系の組立では **ロジック (CoreS3) は USB-C か内蔵 LiPo、サーボ電源は基板仕様に合わせて分離/安定化**する。
